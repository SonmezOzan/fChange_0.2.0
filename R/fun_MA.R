...
#' @param order Order of the FMA process
#' @param kappa Vector of norm of the FMA operators. The length of this vector must be same as the
#' FMA \code{order}
...
#' @export
#'
#' @return Functional Moving Average data sample (class \code{fd}) containing:
#'\item{coefs}{
...
#' and variables
#'}
#'@details This function should be used for a simple functional moving average data generation
...
#' @seealso \code{\link{Data2fd}, \link{fun_IID}}
#
#' @examples # FMA(1) data with 21 fourier basis with a geometric eigenvalue decay
...
fun_MA = function(n , nbasis, order=NULL, kappa=NULL, Sigma = NULL,  basis = NULL, rangeval=c(0,1), ...){
   if (is.null(Sigma)){
      Sigma = (1:nbasis)^-1}
   ...
   if (is.null(order) || order==0){
      ...
      if(order!=0 & is.null(kappa)){
         ...
         for (i in (order+1):(n+l_burnin)){
            coef[,i] = rowSums(sapply(1:order, function(j) Psi.i[[j]]%*% edata[, i-j])) + edata[,i]
         }
